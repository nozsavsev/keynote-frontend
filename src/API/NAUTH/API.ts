...
export const GetDefaultConfig = () => {
  return new NauthApi.Configuration({
    credentials: "include",
    basePath: AppENVConfig.NAUTH_API_BASE!,
  });
};
    ...
    try {
      ...
      if (
        ...
        if (payload.status === "ServerDown" && !window.location.pathname.includes("/500")) {
          const currentUrl = window.location.origin + window.location.pathname + window.location.search;
          window.location.href = new URL(`/500?redirect=${encodeURIComponent(currentUrl)}`, AppENVConfig.NAUTH_FRONTEND_BASE).toString();
        }
      }
